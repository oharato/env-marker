  ...
  try {
    ...
    if (isRibbon) {
      ...
      switch (position) {
        ...
        case 'top-left':
        default:
          banner.style.top = '25px'; banner.style.left = '-50px'; banner.style.transform = 'rotate(-45deg)'; break;
      }
    }
  ...
  try {
    ...
    if (banner && last && last.text && Array.isArray(msg.patterns) && msg.patterns.find((p: string) => p === last.text)) {
      try {
        ...
        const position = msg.bannerPosition || last.position || 'top-left';
    ...
    if (Array.isArray(msg.patterns) && msg.patterns.length > 0) {
      try {
        ...
        if (matched) {
          ...
          const position = msg.bannerPosition || 'top-left';
  ...
  try {
    ...
    for (const key of allSettings) {
      ...
      if (patterns.find(p => p === last.text)) {
        ...
        const position = (any[`${key}_bannerPosition`] as string) || last.position || 'top-left';
